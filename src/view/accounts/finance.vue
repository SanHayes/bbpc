      ...
      <ul class="content_ul">
        ...
        >
          <div class="content_li flex alcenter between">
            ...
            <p class="flex1 tc">{{ item.usdt_price * item.lever_balance || '0.00' | toFixeds }}</p>
...
export default {
  ...
  methods: {
    transIn(index) {
      ...
      let coin = this.changeBalance.filter(t=>{
        return t.currency===that.asset_list[index].currency
      })[0];//this.changeBalance[index]
      let avi = this.$t('center.available')
      // let that = this;
      this.$prompt(`${coin.currency_name} ${avi} ${coin.change_balance} `, this.$t('transferFromChange'), {
        confirmButtonText: this.$t('transferFromChange'),
        cancelButtonText: this.$t('legal.ceil'),
    ...
    transOut(index) {
      ...
      let avi = this.$t('center.available')
      this.$prompt(`${coin.currency_name} ${avi} ${coin.lever_balance} `, this.$t('transferChange'), {
        confirmButtonText: this.$t('transferChange'),
        cancelButtonText: this.$t('legal.ceil'),
        inputPlaceholder: this.$t('account.huanum'),
        inputType: 'number',
        closeOnClickModal: false
    ...
    rec(index, currency) {
      ...
      if (this.flag) {
        ...
        this.$http({
          ...
          method: "post",
          data: {type: "3", currency: currency, page: 1},
      ...
      $.ajax({
        ...
        success: function (res) {
          if (res.type == "ok") {
            ...
            that.min_number_txt = parseFloat(res.message.min_number).toFixed(2);
        ...
        headers: {Authorization: that.token}
          ...
          .then(res => {
            if (res.data.type == "ok") {
              ...
              that.totle = res.data.message.lever_wallet.usdt_totle;
    ...
    moreData() {
      ...
      this.$http({
        url: "/api/wallet/legal_log",
        method: "post",
        data: {type: "3", currency: that.currency, page: that.page},
        headers: {Authorization: this.token}
      }).then(res => {
        if (res.data.type == "ok") {
