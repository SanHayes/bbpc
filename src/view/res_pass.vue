     ...
     <!--  <div class="tip" v-if="true" style="margin-bottom: 10px">
      ...
      <div class="flex" style="align-items: center">
        ...
        <el-input
            ...
            v-model="withdraw_password_confirm"
        >
        </el-input>
      </div>
...
export default {
  data() {
    return {
      ...
      old_password:"", // 原提现密码
      withdraw_password: "", // 新提现密码
      withdraw_password_confirm: ""
    };
  },
  created() {
    this.getCoins();
  ...
  methods: {
    ...
    transfer() {
      let that = this;
      const {old_password, withdraw_password, withdraw_password_confirm} = this
      if (!old_password){
        this.$message.info(this.$t('pass4'))
        return
      }
      if (withdraw_password.length < 6 || withdraw_password.length > 16){
        this.$message.info(this.$t('lay.pwdlength'))
        return
      }
      if (withdraw_password !== withdraw_password_confirm){
        this.$message.info(this.$t('lay.twopwd'))
        return;
      }
      const loading = this.$loading({
        lock: true,
