                ...
                <div class="main" v-if="false">
                    ...
                    <div class="register-input">
                     ...
                     <div class="register-input code-input" >
                        ...
                        <button type="button" @click="setTime">{{$t('forget.getcode')}}</button>
                        </div>
                    </div>
                    <div style="margin-top: 10px;">
                        <span class="register-item"></span>
                        <button class="register-button curPer" type="button" @click="check">{{$t('confirm')}}</button>

                    </div>

                </div>
                <div class="main" v-if="true">
                    <div class="main_title">{{$t('miscro.resetpwd')}}</div>
                  <div class="register-input">
                    <span class="register-item">{{$t('lpwd.oldpwd')}}</span>
                    <input type="password" class="input-main input-content"  v-model="oldpassword" id="opwd">
                  </div>
                    <div class="register-input">
                    ...
                    <button class="register-button curPer" type="button" @click="resetPass" style="margin-top:20px">{{$t('confirm')}}</button>
                </div>
            </div>
        </div>
    </div>
  ...
  methods: {
    ...
    setTime(e) {
      if (e.target.disabled) {
        ...
        if (this.account_number == "") {
          ...
          layer.tips("The mobile phone or email account you entered does not meet the rules!", "#account");
          return;
        }
        ...
        timer = setInterval(function() {
          ...
          if (time == 0) {
            clearInterval(timer);
            e.target.innerHTML = "Verification code";
    ...
    check() {
      ...
      if (user_string == "") {
        ...
        layer.tips("please enter verification code!", "#pwd");
        return;
      } else if (isEmail) {
        ...
        layer.tips("The email or phone number you entered does not meet the rules!", "#account");
        return;
      }
    ...
    resetPass() {
      if (!this.oldpassword){
        layer.msg(this.$t("lpwd.oldpwd"));
        return;
      }
      if (this.password.length < 6 || this.password.length > 16){
        layer.msg(this.$t("lay.pwdlength"));
        return;
      }
      if (this.password == "") {
        ...
        layer.msg("The two passwords entered are inconsistent");
        return;
      } else {
        let data = {
