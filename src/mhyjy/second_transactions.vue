  ...
  <div class="wrap">
    ...
    <ul class="list_content list fColor1 ft12 scroll"
      ...
      <li
        ...
        >0</span>
        <!--status: 1 交易中，2 平仓中， 3已平仓-->
        <!--type： 1 买涨 2 买跌-->
        <!--profit_ratio 最小  max_ratio 最大-->
        <span class="width2 red"
              v-if="item.status == 1&& status == 1 && item.type == 1 && (item.open_price -0) > (prices -0)"
        >-{{ item.number * item.profit_ratio / 100 || '0.00' | numFilters(2) }}</span>
        <span class="width2 green"
              v-if="item.status == 1&& status == 1 && item.type == 1 && (item.open_price -0) <= (prices -0)"
        >{{ (item.number * item.profit_ratio - 0) / 100 || '0.00' | numFilters(2) }}</span>
        <span
            class="width2 green"
